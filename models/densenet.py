...
from models import PatchEmbeddings
...
#        # 1 - For when we are using patch embeddings separately
...
#        )
        ...
        # 3 - For when we are incorporating the patch embedder within the model
        # pooling and classification
        self.class_layers = nn.Sequential(
            OrderedDict(
                [
                    ...
                    ("pool", avg_pool_type(1)),
                    ("flatten", nn.Flatten(1)),
                    ("out", PatchEmbeddings(feature_size=in_channels, num_patches=num_patches, embed_dim=out_channels)),
                ]
            )
        )
