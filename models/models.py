...
class Custom3DCNN(nn.Module):
    ...
    def __init__(self, orig_img_dim, hidden_dim=128):
        ...
        def calc_final_dim(dim):
            # Formula: out = (W - F + 2P)/S + 1
            # W = original dim, F = kernel size, P = padding, S = stride
            return math.floor(math.floor(math.floor(math.floor(math.floor(dim-2+2)/2+1)-3+1)/3+1)/4)

        # Flatten the output and add a fully connected layer to reduce to hidden_dim
        final_dims = [calc_final_dim(i) for i in orig_img_dim]
        self.fc = nn.Linear(hidden_dim * np.prod(final_dims), hidden_dim)
