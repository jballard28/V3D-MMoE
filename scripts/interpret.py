...
def parse_args():
    ...
    parser.add_argument('--whichsamp', type=int, required=True)
...
def main():
    ...
    print(best_model_fus)
    print(best_model_enc)

    # set the evaluation mode
    best_model_fus.eval()
    for encoder in best_model_enc.values():
        encoder.eval()
    ...
    # Apply the modification to the (image encoder) model
    for encoder in best_model_enc.values():
        replace_relu(encoder)
    ...
    # Register the hook to the last convolutional layer of each image encoder
    fhook_mri = best_model_enc['mri'][0].features.transition2.conv.register_forward_hook(save_activations)
    fhook_fdg = best_model_enc['fdg'][0].features.transition2.conv.register_forward_hook(save_activations)
    # Register the hook to the last convolutional layer of each image cross-encoder
    fhook_mri2fdg = best_model_enc['mri2fdg'][0].features.transition2.conv.register_forward_hook(save_activations)
    fhook_fdg2mri = best_model_enc['fdg2mri'][0].features.transition2.conv.register_forward_hook(save_activations)
    ...
    def save_gradient(module, grad_in, grad_out):
        gradients.append(grad_out[0].detach().cpu().numpy().squeeze())
    
    # Register the backward hook on a convolutional layer
    bhook_mri = best_model_enc['mri'][0].features.transition2.conv.register_full_backward_hook(save_gradient)
    bhook_fdg = best_model_enc['fdg'][0].features.transition2.conv.register_full_backward_hook(save_gradient)

    bhook_mri2fdg = best_model_enc['mri2fdg'][0].features.transition2.conv.register_full_backward_hook(save_gradient)
    bhook_fdg2mri = best_model_enc['fdg2mri'][0].features.transition2.conv.register_full_backward_hook(save_gradient)


    # Retrieve samples of interest from dataloader
    # - Class 0: Samples 0,1,4,6,7,10
    # - Class 1: Samples 9,15,26,40,51,53
    # - Class 2: Samples 13,28,35,60,86,91
    test_iterator = iter(test_loader)
    batch_samples, batch_labels, batch_mcs, batch_observed = next(test_iterator)
    ...
    whichsamp = args.whichsamp
    samp = {k: v[whichsamp:whichsamp+1] for k, v in batch_samples.items()}
    lbl = batch_labels[whichsamp:whichsamp+1]
    mc = batch_mcs[whichsamp:whichsamp+1]
    obs = batch_observed[whichsamp:whichsamp+1]

    # Apply the model
    input_shapes = {}
    orig_input = {}
    encoder_order = []
    mods = [m for (m,s) in samp.items()]
    fusion_input_dict = {}
    cross_enc = {m:{} for m in samp.keys()} # {resulting_mod: {source_mod: embeddings}}
    masks = {}
    for i, (modality, samples) in enumerate(samp.items()):
        input_shapes[modality] = samples.shape
        orig_input[modality] = samples.detach().cpu().numpy()
        mask = obs[:, modality_dict[modality]]
        masks[modality] = mask
        encoded_samples = torch.full((samples.shape[0], args.num_patches, args.hidden_dim), torch.nan, dtype=torch.float).to(device)
        if mask.sum() > 0:
            encoded_samples[mask] = best_model_enc[modality](samples[mask])
            encoder_order.append(modality)
        ...
        if modality in ['mri','fdg']:
            for jj in range(len(mods)):
                if jj != i and mods[jj] in ['mri','fdg']:
                    ...
                    if mask.sum() > 0:
                        othermod = mods[jj]
                        crossenc_samples[mask] = best_model_enc[modality+'2'+othermod](samples[mask])
                        encoder_order.append(modality+'2'+othermod)
                    cross_enc[othermod][modality] = crossenc_samples
        fusion_input_dict[modality] = encoded_samples
    ...
    outputs = best_model_fus(*fusion_input, expert_indices=mc)

    # Pick class with the highest score
    ...
    fhook_mri.remove()
    fhook_fdg.remove()
    fhook_mri2fdg.remove()
    fhook_fdg2mri.remove()
    bhook_mri.remove()
    bhook_fdg.remove()
    bhook_mri2fdg.remove()
    bhook_fdg2mri.remove()

    act_shape = np.shape(activations[0])
    print(f"Shape of activations[0]: {act_shape}") # (512, 14, 14)
    act_shape = np.shape(activations[1])
    print(f"Shape of activations[1]: {act_shape}") # (512, 14, 14)

    grad_shape = np.shape(gradients[0])
    print(f"Shape of gradients[0]: {grad_shape}") # (512, 14, 14)
    grad_shape = np.shape(gradients[1])
    print(f"Shape of gradients[1]: {grad_shape}") # (512, 14, 14)

    # Reset gradients
    best_model_fus.zero_grad()
    for encoder in best_model_enc.values():
        encoder.zero_grad()
    ...
    all_probs.extend(torch.nn.functional.softmax(outputs, dim=1).detach().cpu().numpy())
    all_obs.append(obs.detach().cpu().numpy())

    gradcam_info = {'activations':activations, 'gradients':gradients, 'preds':all_preds, 'labels':all_labels, 'probs':all_probs, 'all_obs':all_obs, 'input_shapes':input_shapes, 'orig_input':orig_input, 'encoder_order':encoder_order}

    # Save gradients, activations, predictions, labels, and probs to pickle
    output_dir = '../interpretation_results'
    with open(os.path.join(output_dir,'gradcam_info_'+str(whichsamp)+'.pickle'), 'wb') as f:
        pickle.dump(gradcam_info, f)
